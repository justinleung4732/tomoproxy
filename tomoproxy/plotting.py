...
def plot_err_coefs(err_coefs, r=None, title='', quantity='', cmap='Greys',
                      ...
                      show_cb=True, return_map = False):
    """
    Plot the errors of spherical harmonic coefficients `coef`.  Label the radius with 
    `r` (km) and the value with `quantity`.  Harmonics are truncated at degree `lmax`.
    If `fig` and `ax` are supplied, then the plot is added to the provided matplotlib
    figure and axis handles, respectively.
    """
    assert ( (ax is not None) and (fig is not None) or
    ...
    err_grid = _make_2d_err_grid(err_coefs, interval)
    if scale_factor is not None:
...
def _make_2d_err_grid(coefs, interval):
    """
    Expand a grid of spherical harmonic coefficient errors with squared Legendre
    polynomials.
    """
